...
gaming_laptops_df = pd.read_csv("gaming_laptops_raw.csv", sep=",")
...
for i in range(len(gaming_laptops_df["rating"])):
    ...
    print(gaming_laptops_df["rating"])

# In[271]:


gaming_laptops_df = gaming_laptops_df.fillna(-1)
print(gaming_laptops_df.cpu_speed)
...
# Cleaning Gaming Laptop's Number of Cores
gaming_laptops_df.cores.unique()
...
# Recommending Laptops based on GPU
gaming_laptops_df.gpu_type.unique()

# In[276]:


for i in range(len(gaming_laptops_df["gpu_type"])):
    if "-1" in gaming_laptops_df["gpu_type"]:
        pass
...
# Cleaning Gaming Laptops' based on GPU's memory
gaming_laptops_df.gpu_memory.unique()
